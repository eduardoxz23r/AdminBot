...
@Component
public class LockCommand extends ListenerAdapter implements SlashCommandHandler {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (!event.getName().equals("lock")) return;
        handle(event);
    }
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        if (permOverride != null && permOverride.getDenied().contains(Permission.MESSAGE_SEND)) {
            event.getHook().sendMessage("⚠️ Este canal já está travado.").queue();
            return;
        }
        ...
        channel.getManager()
                ...
                .queue(
                        success -> {
                            ...
                            event.getHook().sendMessage("✅ Canal **#" + channel.getName() + "** travado com sucesso!").queue();
                        },
                        error -> event.getHook().sendMessage("❌ Erro ao travar: " + error.getMessage()).queue()
                );
    }
}